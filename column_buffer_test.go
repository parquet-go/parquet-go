...
// TestByteArrayColumnBufferWriteTypes tests that byteArrayColumnBuffer
// correctly converts various types to text representation
func TestByteArrayColumnBufferWriteTypes(t *testing.T) {
	tests := []struct {
		name     string
...
// TestByteArrayColumnBufferWriteInt96 tests writeInt96 which converts
// to big.Int text representation
func TestByteArrayColumnBufferWriteInt96(t *testing.T) {
	col := newByteArrayColumnBuffer(ByteArrayType, 0, 10)

	// Write an Int96 value - it will be converted to big.Int text format
	...
	actual := string(col.index(0))
	// Int96 gets converted to big.Int, so we just verify it's a valid number
	if len(actual) == 0 {
		t.Errorf("expected non-empty string representation")
	}
	t.Logf("Int96 %v converted to: %q", val, actual)
}

// TestByteArrayColumnBufferWriteMultipleValues tests writing multiple values
func TestByteArrayColumnBufferWriteMultipleValues(t *testing.T) {
	col := newByteArrayColumnBuffer(ByteArrayType, 0, 10)
...
// TestByteArrayColumnBufferReadWriteRoundTrip tests that values written
// can be read back correctly
func TestByteArrayColumnBufferReadWriteRoundTrip(t *testing.T) {
	col := newByteArrayColumnBuffer(ByteArrayType, 0, 10)
